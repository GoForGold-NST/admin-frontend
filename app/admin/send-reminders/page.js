...
export default function SendRemindersPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${BACKEND_URL}/admin/send-reminders`, {
        method: 'POST',
        headers: {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-2xl font-bold text-black">Send Payment Reminders</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <p className="mt-2 text-sm text-black">
              CSV should contain a column with email addresses of participants to send reminders to.
            </p>
          </div>
          ...
          <div>
            ...
            >
              {uploading ? 'Sending...' : 'Send Reminders'}
            </button>
          </div>
        </form>
      </div>
      ...
      {result && (
        <div className="bg-white p-6 rounded-lg shadow">
          ...
          {result.reminders && result.reminders.length > 0 && (
            <div className="mt-6">
              <h4 className="text-md font-medium text-gray-900 mb-2">Reminder Details</h4>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {result.reminders.map((reminder, index) => (
                      <tr key={index}>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-black">{reminder.email}</td>
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-black">{reminder.name}</td>
